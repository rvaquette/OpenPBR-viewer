...
vec3 diffuseColor = vec3(0.400000, 0.200000, 0.550000);
vec3 emissiveColor = vec3(0.000000, 0.000000, 0.000000);
int useSpecularWorkflow = 0;
vec3 specularColor = vec3(0.000000, 0.000000, 0.000000);
float metallic = 0.000000;
float roughness = 0.450000;
float clearcoat = 0.000000;
float clearcoatRoughness = 0.010000;
float opacity = 1.000000;
int opacityMode = 0;
float opacityThreshold = 0.000000;
float ior = 1.500000;
vec3 normal = vec3(0.000000, 0.000000, 1.000000);
float displacement = 0.000000;
...
surfaceshader mtlxHostEvalSurface()
{
    ...
    IMP_UsdPreviewSurface_surfaceshader(diffuseColor, emissiveColor, useSpecularWorkflow, specularColor, metallic, roughness, clearcoat, clearcoatRoughness, opacity, opacityMode, opacityThreshold, ior, normal, displacement, occlusion, SR_synthetic_out);
    return SR_synthetic_out;
}


// Generated pathtracer dispatch (feature 003). Model: UsdPreviewSurface
...
vec3 sampleBsdf(in vec3 pW, in Basis basis, in vec3 winputL, inout uint rndSeed, in int material, out vec3 woutputL, out float pdf_woutputL, out Volume internal_medium)
{
    ...
    float m_aniso = clamp(0.0, 0.0, 0.99);
    vec3  m_base  = (diffuseColor) * (1.0);
    vec3  m_specC = vec3(1.0);
    float m_specW = 1.0;
    float m_ior   = max(ior, 1.0 + 1e-3);
    float m_coatW = clamp(0.0, 0.0, 1.0);
    float m_coatRough = clamp(0.0, 0.0, 1.0);
    ...
    float pTrans = 0.0;
    float m_transW = clamp(0.0, 0.0, 1.0);
