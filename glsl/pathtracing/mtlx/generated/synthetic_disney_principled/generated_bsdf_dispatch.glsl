...
vec3 baseColor = vec3(0.550000, 0.350000, 0.100000);
float metallic = 0.000000;
float roughness = 0.350000;
...
float clearcoatGloss = 1.000000;
float specTrans = 0.000000;
float ior = 1.500000;
float subsurface = 0.000000;
vec3 subsurfaceDistance = vec3(1.000000, 1.000000, 1.000000);
// __MTLX_PARAMS_END__
...
void NG_disney_principled(vec3 baseColor, float metallic, float roughness, float anisotropic, float specular, float specularTint, float sheen, float sheenTint, float clearcoat, float clearcoatGloss, float specTrans, float ior, float subsurface, vec3 subsurfaceDistance, out surfaceshader out1)
{
    const float coat_intensity_in2_tmp = 0.040000;
    float coat_intensity_out = clearcoat * coat_intensity_in2_tmp;
    const float coat_roughness_amount_tmp = 1.000000;
    ...
    const float metallic_mix_fg_weight_in1_tmp = 1.000000;
    float metallic_mix_fg_weight_out = metallic_mix_fg_weight_in1_tmp * metallic;
    vec2 specular_roughness_out = vec2(0.0);
    mx_roughness_anisotropy(roughness, anisotropic, specular_roughness_out);
    const float dielectric_intensity_in2_tmp = 0.080000;
    float dielectric_intensity_out = specular * dielectric_intensity_in2_tmp;
    const vec3 dielectric_tint_bg_tmp = vec3(1.000000, 1.000000, 1.000000);
    vec3 dielectric_tint_out = mix(dielectric_tint_bg_tmp, baseColor, specularTint);
    const float transmission_mix_fg_weight_in1_tmp = 1.000000;
    float transmission_mix_fg_weight_out = transmission_mix_fg_weight_in1_tmp * specTrans;
    const vec3 sheen_color_bg_tmp = vec3(1.000000, 1.000000, 1.000000);
    vec3 sheen_color_out = mix(sheen_color_bg_tmp, baseColor, sheenTint);
    const float subsurface_mix_fg_weight_in1_tmp = 1.000000;
    float subsurface_mix_fg_weight_out = subsurface_mix_fg_weight_in1_tmp * subsurface;
    const float invert_metalness_amount_tmp = 1.000000;
    ...
    float transmission_mix_mix_inv_out = transmission_mix_mix_inv_amount_tmp - specTrans;
    const float metallic_mix_mix_inv_amount_tmp = 1.000000;
    float metallic_mix_mix_inv_out = metallic_mix_mix_inv_amount_tmp - metallic;
    vec2 coat_roughness_vector_out = vec2(0.0);
    NG_convert_float_vector2(coat_roughness_out, coat_roughness_vector_out);
    float subsurface_mix_bg_weight_out = invert_metalness_out * subsurface_mix_mix_inv_out;
    vec2 coat_roughness_dual_out = vec2(0.0);
    ...
    surfaceshader surface_constructor_out = surfaceshader(vec3(0.0),vec3(0.0));
    {
        ...
        mx_generalized_schlick_bsdf(closureData, coat_intensity_out, vec3(1.000000, 1.000000, 1.000000), vec3(1.000000, 1.000000, 1.000000), vec3(1.000000, 1.000000, 1.000000), 5.000000, coat_roughness_dual_out, false, 0.000000, 1.500000, geomprop_Nworld_out1, geomprop_Tworld_out1, 0, 0, coat_bsdf_out);
        BSDF metallic_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
        mx_generalized_schlick_bsdf(closureData, metallic_mix_fg_weight_out, baseColor, vec3(1.000000, 1.000000, 1.000000), vec3(1.000000, 1.000000, 1.000000), 5.000000, specular_roughness_out, false, 0.000000, 1.500000, geomprop_Nworld_out1, geomprop_Tworld_out1, 0, 0, metallic_bsdf_out);
        BSDF dielectric_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
        mx_generalized_schlick_bsdf(closureData, dielectric_intensity_out, dielectric_tint_out, vec3(1.000000, 1.000000, 1.000000), vec3(1.000000, 1.000000, 1.000000), 5.000000, specular_roughness_out, false, 0.000000, 1.500000, geomprop_Nworld_out1, geomprop_Tworld_out1, 0, 0, dielectric_bsdf_out);
        BSDF transmission_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
        mx_dielectric_bsdf(closureData, transmission_mix_fg_weight_out, baseColor, ior, vec2(0.000000, 0.000000), false, 0.000000, 1.500000, geomprop_Nworld_out1, geomprop_Tworld_out1, 0, 1, transmission_bsdf_out);
        BSDF sheen_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
        mx_sheen_bsdf(closureData, sheen, sheen_color_out, 0.300000, geomprop_Nworld_out1, 0, sheen_bsdf_out);
        BSDF subsurface_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
        mx_subsurface_bsdf(closureData, subsurface_mix_fg_weight_out, baseColor, subsurfaceDistance, 0.000000, geomprop_Nworld_out1, subsurface_bsdf_out);
        BSDF diffuse_bsdf_out = BSDF(vec3(0.0),vec3(1.0));
        mx_burley_diffuse_bsdf(closureData, subsurface_mix_bg_weight_out, baseColor, roughness, geomprop_Nworld_out1, diffuse_bsdf_out);
        BSDF subsurface_mix_add_out = BSDF(vec3(0.0),vec3(1.0));
        mx_add_bsdf(closureData, subsurface_bsdf_out, diffuse_bsdf_out, subsurface_mix_add_out);
        ...
        BSDF dielectric_layer_out = BSDF(vec3(0.0),vec3(1.0));
        mx_layer_bsdf(closureData, dielectric_bsdf_out, transmission_mix_add_out, dielectric_layer_out);
        BSDF metallic_mix_bg_mul_out = BSDF(vec3(0.0),vec3(1.0));
        mx_multiply_bsdf_float(closureData, dielectric_layer_out, metallic_mix_mix_inv_out, metallic_mix_bg_mul_out);
        BSDF metallic_mix_add_out = BSDF(vec3(0.0),vec3(1.0));
        mx_add_bsdf(closureData, metallic_bsdf_out, metallic_mix_bg_mul_out, metallic_mix_add_out);
        BSDF coat_layer_out = BSDF(vec3(0.0),vec3(1.0));
        mx_layer_bsdf(closureData, coat_bsdf_out, metallic_mix_add_out, coat_layer_out);
        surface_constructor_out.color += coat_layer_out.response;
...
surfaceshader mtlxHostEvalSurface()
{
    ...
    surfaceshader SR_synthetic_out = surfaceshader(vec3(0.0),vec3(0.0));
    NG_disney_principled(baseColor, metallic, roughness, anisotropic, specular, specularTint, sheen, sheenTint, clearcoat, clearcoatGloss, specTrans, ior, subsurface, subsurfaceDistance, SR_synthetic_out);
    return SR_synthetic_out;
}


// Generated pathtracer dispatch (feature 003). Model: disney_principled
...
vec3 sampleBsdf(in vec3 pW, in Basis basis, in vec3 winputL, inout uint rndSeed, in int material, out vec3 woutputL, out float pdf_woutputL, out Volume internal_medium)
{
    ...
    float m_metal = clamp(metallic, 0.0, 1.0);
    float m_rough = clamp(roughness, 0.0, 1.0);
    float m_aniso = clamp(anisotropic, 0.0, 0.99);
    vec3  m_base  = (baseColor) * (1.0);
