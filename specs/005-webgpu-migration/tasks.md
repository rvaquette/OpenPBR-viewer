...
- [X] T052.7 Rendre les cinq fixtures carpaint (`carpaint`, `glass`, `pearl`, `soapbubble`, `synthetic_default`): le runner isolé `validate:webgpu-materialx-carpaint` conserve un rapport et un éventuel code `MTLX_WGSL_*` par fixture, puis agrège les cinq résultats dans `t052.7-carpaint-fixtures-report.json`. Le compute host appelle désormais `mtlxGenSampleBsdf` et transporte réflexion/transmission vers l'environnement avec pondération BSDF*cos/pdf; `glass` et `soapbubble` ne sont plus des silhouettes noires (luminance centrale `39.30`/`39.22`). Les cinq passent avec closures générées intactes, pipeline/bind group MaterialX actifs, deux samples, captures 256x256 non uniformes et objet visible, `gpuError=null`, `fallback=false`, aucun code d'erreur. Durées pipeline/total (ms): carpaint `200224/219416.8`, glass `205153.6/234328.0`, pearl `234197.1/256296.1`, soapbubble `241310.8/263123.5`, default `216946.9/241542.0`. La parité physique fine de transmission/thin-film avec l'HDR natif reste explicitement aux gates T053/T054-T055.
