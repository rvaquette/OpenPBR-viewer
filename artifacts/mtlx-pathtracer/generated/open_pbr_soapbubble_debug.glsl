...
vec3 base_color = vec3(0.800000, 0.800000, 0.800000);
...
float specular_roughness = 0.000000;
float specular_ior = 1.000000;
float specular_roughness_anisotropy = 0.000000;
float transmission_weight = 1.000000;
...
float thin_film_weight = 1.000000;
...
bool geometry_thin_walled = true;
// __MTLX_PARAMS_END__
...
// Generated pathtracer dispatch (feature 003). Model: open_pbr_surface
...
vec3 sampleBsdf(in vec3 pW, in Basis basis, in vec3 winputL, inout uint rndSeed, in int material, out vec3 woutputL, out float pdf_woutputL, out Volume internal_medium)
{
    ...
    float m_metal = clamp(base_metalness, 0.0, 1.0);
    float m_rough = clamp(specular_roughness, 0.0, 1.0);
    float m_aniso = clamp(specular_roughness_anisotropy, 0.0, 0.99);
    vec3  m_base  = (base_color) * (base_weight);
    vec3  m_specC = specular_color;
    float m_specW = specular_weight;
    float m_ior   = max(specular_ior, 1.0 + 1e-3);
    float m_coatW = clamp(coat_weight, 0.0, 1.0);
    float m_coatRough = clamp(coat_roughness, 0.0, 1.0);
    float m_coatAniso = clamp(coat_roughness_anisotropy, 0.0, 0.99);
    ...
    float pTrans = 0.0;
    float m_transW = clamp(transmission_weight, 0.0, 1.0);
